...
| C4.1.2 | Sondes, seuils, signalement et disponibilité | Sonde santé 15 min, seuil latence, SLO 30 jours, incidents uniques | `npm run monitoring:probe` et `npm run monitoring:slo` | `B4-C412-04` |
...
3. Le manifeste `preuves/MANIFEST.sha256` couvre le dossier, le PDF, les politiques, les scripts, les workflows, les tests et les preuves ; toute dérive de contenu échoue.
...
Les sept compétences sont **industrialisées et vérifiées**. Les prochaines actions de maintenance restent séparées de ce constat : traiter les futures alertes, enregistrer les retours réellement reçus et ne promouvoir une nouvelle version qu'après les portes de release prévues.
