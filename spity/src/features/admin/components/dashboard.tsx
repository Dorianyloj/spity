    ...
    <div className="grid gap-5 xl:grid-cols-3">
      <section className={`${panelClass} xl:col-span-2`} aria-labelledby="activity-title">
        ...
        <svg viewBox="0 0 680 260" className="mt-5 w-full" role="img" aria-labelledby="chart-title chart-description">
          ...
          {[0, 1, 2, 3, 4].map((tick) => <g key={tick}><line x1="40" x2="670" y1={220 - tick * 50} y2={220 - tick * 50} stroke="#ccd9c7" /><text x="30" y={225 - tick * 50} textAnchor="end" className="text-[24px] sm:text-[12px]" fill="#55645b">{number(ceiling * tick / 4)}</text></g>)}
          {buckets.map((point, index) => {
            const width = 620 / buckets.length
            const x = 45 + width * index
            return <g key={point.start}><rect x={x + width * .15} y={220 - point.posts / ceiling * 200} width={width * .27} height={point.posts / ceiling * 200} rx="3" fill="#173236" /><rect x={x + width * .48} y={220 - point.interactions / ceiling * 200} width={width * .27} height={point.interactions / ceiling * 200} rx="3" fill="#6d983b" /><text x={x + width * .45} y="246" textAnchor="middle" className="text-[24px] sm:text-[12px]" fill="#55645b">{shortDate(point.start)}</text></g>
          })}
        </svg>
