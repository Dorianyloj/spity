...
export function ProfileOverview({ profile, onEdit, fallbackAction, posts }: { profile: PublicProfile; onEdit?: (kind: ProfileEditorKind) => void; fallbackAction: ReactNode; posts: ReactNode }) {
  ...
  return <div className="space-y-6">
    <ProfileCard title="Ma pratique" icon={Mountain} action={edit('practice')}>
      {profile.disciplines.length ? <><dl className="grid divide-y divide-border overflow-hidden rounded-xl border border-border border-t-4 border-t-primary bg-muted/20 sm:grid-cols-3 sm:divide-x sm:divide-y-0">{profile.disciplines.map((key) => <div key={key} className="min-w-0 px-4 py-5 sm:px-5"><dt className="wrap-anywhere text-sm font-semibold text-muted-foreground">{disciplineLabels[key] ?? key}</dt><dd className="mt-2 text-4xl font-bold tabular-nums text-foreground">{profile.niveaux[key] || '—'}</dd></div>)}</dl><p className="mt-4 text-pretty text-xs text-muted-foreground">Repères personnels, pas un classement.</p></> : <ProfileEmpty title="À chacun sa pratique" action={onEdit ? edit('practice', 'Renseigner ma pratique') : fallbackAction}>Les disciplines et les niveaux ne sont pas encore renseignés.</ProfileEmpty>}
    </ProfileCard>
