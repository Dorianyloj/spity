...
export function ProfilePosts({ profile, summary = false, basePath, onCompose, onAll }: { profile: PublicProfile; summary?: boolean; basePath: string; onCompose?: () => void; onAll?: () => void }) {
  ...
  return <section className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3 text-white"><h2 className="text-balance text-xl font-bold">{summary ? 'Au fil des sessions' : 'Publications'}</h2>{summary ? onAll && <Button variant="secondary" onClick={onAll}>Tout voir</Button> : onCompose && <Button onClick={onCompose}><Plus size={16} aria-hidden="true" />Créer une publication</Button>}</div>
