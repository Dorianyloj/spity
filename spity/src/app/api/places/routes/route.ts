...
import { cragRouteInputSchema } from '@/features/places/schemas'
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = cragRouteInputSchema.safeParse(await readProfileBody(request))
    if (!parsed.success) {
      return privateProfileResponse({
        error: 'Vérifie les informations de la voie.',
        issues: parsed.error.issues.map(({ path, message }) => ({ path: path.join('.'), message })),
      }, 422)
    }

    const limit = consumeRateLimit(`crag-routes:${profile.user.id}`, {
      bucket: 'api',
      maxRequests: 12,
      windowMs: 15 * 60 * 1000,
    })
    if (!limit.allowed) {
      return privateProfileResponse({ error: 'Trop de voies ajoutées. Réessaie dans quelques minutes.' }, 429)
    }

    const route = await createCragRoute(profile.user.id, parsed.data)
    return privateProfileResponse({ route }, 201)
  } catch (error) {
