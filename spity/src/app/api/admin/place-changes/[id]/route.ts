...
import { reviewPlaceChangeRequest } from '@/features/admin/lib/repository'
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!input.success) return adminResponse({ error: input.error.issues[0]?.message ?? 'Décision invalide.' }, 422)
    await reviewPlaceChangeRequest(id.data, input.data)
    return adminResponse({ ok: true })
  } catch (error) {
    if (error instanceof AdminError) return adminResponse({ error: error.message }, error.status)
    logger.error('admin.place_change_review_failed', { reason: 'internal_error' })
