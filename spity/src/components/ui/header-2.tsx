...
function HeaderBrand() {
  return <span className="relative block h-12 w-24 shrink-0 overflow-hidden">
    <Image src={brandAssets.logoTransparent} alt="" width={128} height={128} priority className="absolute top-1/2 left-1/2 size-32 max-w-none -translate-x-1/2 -translate-y-1/2" />
  </span>
}
...
function PublicActions() {
  return <>
    ...
    <Link href="/register" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-secondary">
...
export function Header({ links = publicLinks, homeHref = '/', accountLabel, actions, mobileActions }: {
  links?: HeaderLink[]
  homeHref?: string
  accountLabel?: string
  actions?: ReactNode
  ...
  return <header data-spity-header data-scrolled={scrolled} className="sticky top-0 z-40 px-3 pt-[max(0.75rem,env(safe-area-inset-top))] sm:px-6">
    <div className={cn('mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-2xl border p-2.5 text-foreground sm:px-4', scrolled ? 'border-border bg-card/95 shadow-lg backdrop-blur-lg' : 'border-card/80 bg-card shadow-sm')}>
    ...
    {open && <dialog ref={dialogRef} id={id} aria-labelledby={`${id}-title`} onCancel={(event) => { event.preventDefault(); close() }}
      ...
      <div className="mx-auto flex min-h-full max-w-xl flex-col pl-[max(1.25rem,env(safe-area-inset-left))] pr-[max(1.25rem,env(safe-area-inset-right))] pt-[max(1.25rem,env(safe-area-inset-top))] pb-[max(1.25rem,env(safe-area-inset-bottom))]">
        <div className="flex items-center justify-between gap-4 border-b border-border pb-5">
          <Link href={homeHref} onNavigate={close} aria-label="Accueil Spity" className="flex min-h-11 items-center rounded-xl"><HeaderBrand /></Link>
